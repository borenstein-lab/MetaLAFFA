...
parser.add_argument("--submission_wrapper", "-s", default=" ".join([op.python, cl.submission_wrapper]), help="The wrapper script to use to parse job settings and resource requests and submit a job to the cluster (default %(default)s)")
parser.add_argument("--jobscript", "-j", default=cl.jobscript, help="The job script to use when an individual cluster job (default %(default)s)")
