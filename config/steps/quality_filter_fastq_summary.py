"""
Quality filter FASTQ summary step parameters
---------------------------

This configuration submodule contains parameters related to the quality filter fastq summary pipeline step.
"""
...
step_prefix = "quality_filter_fastq_summary"
"""
...
def default(inputs, outputs, wildcards):
    ...
    if not lf.is_empty(inputs.input):
        subprocess.run([op.python, fo.source_directory + "quality_filter_fastq_summary.py", inputs.input, "--output", outputs[0], "--use_sample", "--use_type"], env=env)
    else:
