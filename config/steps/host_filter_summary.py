"""
Host filter summary step parameters
---------------------------

This configuration submodule contains parameters related to the host filter summary pipeline step.
"""
...
step_prefix = "host_filter_summary"
"""
...
def default(inputs, outputs, wildcards):
    ...
    if not lf.is_empty(inputs.input):
        subprocess.run([op.python, fo.source_directory + "host_filter_summary.py", inputs.input, "--output", outputs[0], "--use_sample", "--use_type"], env=env)
    else:
